...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (!error.response) {
      ...
      // Also, don't log out for endpoints that might be called before login (onboarding, products list)
      if (error.response.status === 401 && !isStepUpAuth && !isPermissionError) {
        ...
        // and if we have a token (meaning we were logged in but token expired)
        const token = localStorage.getItem('token');
        if (token && window.location.pathname !== '/login' && !error.config?.url?.includes('/api/auth/token/')) {
