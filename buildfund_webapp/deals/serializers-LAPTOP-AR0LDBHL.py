...
class DealSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Deal
        fields = [
            'id', 'deal_id', 'application', 'application_details', 'lender', 'lender_name',
...
class DealTaskSerializer(serializers.ModelSerializer):
    ...
    def get_dependencies_list(self, obj):
        """Get list of dependency task titles."""
        return [{'id': dep.id, 'title': dep.title, 'status': dep.status} for dep in obj.dependencies.all()]
    
    def get_blocked_by(self, obj):
        """Get list of tasks that block this task (incomplete dependencies)."""
        incomplete_deps = obj.dependencies.filter(status__in=['pending', 'in_progress', 'blocked'])
        return [{'id': dep.id, 'title': dep.title, 'status': dep.status} for dep in incomplete_deps]
    ...
    class Meta:
        model = DealTask
        fields = [
            'id', 'deal', 'stage', 'stage_name', 'title', 'description', 'owner_party_type',
            'assignee_user', 'assignee_party', 'assignee_name', 'due_date',
            'sla_hours', 'dependencies', 'dependencies_list', 'blocked_by',
            'status', 'status_display', 'priority', 'priority_display',
            'required_docs', 'blockers', 'is_overdue',
