...
class ReportInputBuilder:
    ...
    def build(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'lender_and_product_data': self._build_lender_and_product_data(),
    ...
    def _build_directors_shareholders(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'shareholders_pscs': [
                {
                    'name': s.get('name', 'Not provided'),
                    'ownership_percentage': float(s.get('ownership_percentage', 0)) if s.get('ownership_percentage') else (
                        float(s.get('natures_of_control', [{}])[0].get('percentage', 0)) if s.get('natures_of_control') else 0
                    ),
    ...
    def _build_bank_data_summary(self) -> Dict[str, Any]:
        ...
        if method == 'open_banking':
            ...
            return {
                ...
                'accounts': [
                    {
                        'account_name': self._mask_account(acc.get('account_name', 'Account')),
                        'account_number_masked': self._mask_account_number(acc.get('account_number', '')),
                        'balance': float(acc.get('balance', 0)) if acc.get('balance') else None,
                        'account_type': acc.get('account_type', 'Not provided'),
                    }
                    for acc in accounts
                ],
                'summary': {
                    'total_balance': sum(float(acc.get('balance', 0)) for acc in accounts if acc.get('balance')),
    ...
    def _build_lender_and_product_data(self) -> Dict[str, Any]:
        ...
        return {
            'lender': {
                'name': self.lender.company_name or self.lender.legal_name or 'Not provided',
                ...
                'lender_type': self.lender.lender_type or 'Not provided',
            },
            'product': {
                ...
                'interest_rate_range': product.interest_rate_range if product else 'Not provided',
                'ltv_range': product.ltv_range if product else 'Not provided',
