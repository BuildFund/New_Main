        ...
        if self.status != new_status:
            ...
            if old_status != 'submitted' and new_status == 'submitted':
                try:
                    ...
                    service.generate_report(self, created_by=None)  # System-generated
            ...
            # Create Deal if status changed to "accepted"
            if old_status != 'accepted' and new_status == 'accepted':
                try:
                    from deals.services import DealService
                    deal = DealService.create_deal_from_application(self)
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.info(f"Deal {deal.deal_id} created from application {self.id}")
                except Exception as e:
                    # Log error but don't fail the status update
                    import logging
                    logger = logging.getLogger(__name__)
...
class UnderwriterReport(models.Model):
    """Stores comprehensive lender-ready underwriter's reports for applications."""
    ...
    version = models.PositiveIntegerField(default=1, help_text="Version number for this report")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='generating')
    is_locked = models.BooleanField(default=False, help_text="If locked, report cannot be regenerated")
    
    # Report data
    report_json = models.JSONField(default=dict, help_text="Full structured JSON report")
    plain_text_narrative = models.TextField(blank=True, help_text="Human-readable narrative version")
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="User who triggered report generation (admin or system)"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    input_data_snapshot = models.JSONField(default=dict, blank=True, help_text="Snapshot of input data used")
    ...
    def __str__(self) -> str:
        return f"UnderwriterReport(app={self.application.id}, lender={self.lender.id}, v{self.version})"
    
    @property
    def is_latest(self):
        """Check if this is the latest version for this application/lender."""
...
class InformationRequest(models.Model):
    """Represents a structured information request from lender to borrower."""
    ...
    title = models.CharField(max_length=255, help_text="Title of the information request")
    notes = models.TextField(blank=True, help_text="Additional notes or instructions")
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        help_text="Lender user who created this request"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self) -> str:
        return f"InformationRequest({self.application.id} - {self.title})"


class InformationRequestItem(models.Model):
    """Individual item within an information request."""
    ...
    title = models.CharField(max_length=255, help_text="Item title")
    description = models.TextField(blank=True, help_text="Detailed description of what is required")
    due_date = models.DateField(null=True, blank=True, help_text="Due date for this item")
    document_type = models.ForeignKey(
        "documents.DocumentType",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        help_text="Required document type (if applicable)"
    )
    ...
    uploaded_document = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='information_request_items',
        help_text="Document uploaded by borrower for this item"
    )
    lender_comment = models.TextField(
        blank=True,
        help_text="Lender's comment when accepting or rejecting"
    )
    reviewed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='reviewed_information_items',
        help_text="Lender user who reviewed this item"
    )
    reviewed_at = models.DateTimeField(null=True, blank=True)
    rework_count = models.PositiveIntegerField(
        default=0,
        help_text="Number of times this item has been rejected and re-uploaded"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self) -> str:
        return f"InformationRequestItem({self.request.id} - {self.title})"