...
class BorrowerProfileViewSet(
    ...
    def submit_for_review(self, request):
        ...
        if 'step2' in data:
            step2 = data['step2']
            profile.company_data = step2.get('company_data', {})
            profile.company_verified_at = timezone.now() if step2.get('confirmed') else None
            profile.trading_address = step2.get('trading_address', '')
            profile.primary_contact_email = step2.get('primary_contact_email', '')
            profile.primary_contact_phone = step2.get('primary_contact_phone', '')
