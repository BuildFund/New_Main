...
class CompanyDataProcessor:
    ...
    def process_and_save_company_data(self, borrower_profile, company_number: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 6. Store full company data - merge with existing data to preserve structure
            existing_company_data = borrower_profile.company_data or {}
            if isinstance(existing_company_data, str):
                # Handle case where company_data might be stored as JSON string
                import json
                try:
                    existing_company_data = json.loads(existing_company_data)
                except (json.JSONDecodeError, TypeError):
                    existing_company_data = {}
            
            # Merge company info into existing data structure
            borrower_profile.company_data = {
                **existing_company_data,
                **company_info,  # Spread company_info fields directly
                "company_number": company_number,
                "last_updated": timezone.now().isoformat(),
            }
            borrower_profile.company_verified_at = timezone.now()
            if not borrower_profile.registration_number:
                borrower_profile.registration_number = company_number
            if not borrower_profile.company_name:
            ...
            logger.error(f"Error processing company data for {company_number}: {e}", exc_info=True)
            results["errors"].append(f"Error processing company data: {str(e)}")
            # Don't return early - allow partial results to be returned
